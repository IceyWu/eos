...
const labelStyle = { color: "var(--eos-color-foreground-muted, #71717a)", font: "500 12px/16px var(--eos-font-family, system-ui)" };
...
export default function Basic() {
	...
	useEffect(() => {
		...
		const onClick = (event: Event) => setCurrent((event as CustomEvent<{ index: number }>).detail.index);
	...
	return (
		<div style={{ display: "grid", gap: 20, maxWidth: 560, width: "100%" }}>
			<div style={{ display: "grid", gap: 8 }}>
				...
				<eos-progress-bar ref={ref} total="4" current={String(current)} variant="default" />
			</div>
			<div style={{ display: "grid", gap: 8 }}>
			...
			<small style={{ color: "var(--eos-color-foreground-muted, #71717a)", textAlign: "center" }}>Active segment: {current + 1} / 4</small>
		</div>
	);
}
