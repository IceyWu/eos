...
export default function Basic() {
	...
	const [placeholderType, setPlaceholderType] = useState<"blurhash" | "url">(
		"blurhash",
	);
		...
		>
			<div style={{ display: "grid", justifyItems: "center", gap: 12 }}>
				<eos-image
					...
					style={{
						aspectRatio: circle ? "1 / 1" : "4 / 3",
						borderRadius: circle ? "50%" : 12,
						height: "auto",
						maxHeight: 300,
						width: responsive ? "100%" : circle ? 220 : "min(100%, 390px)",
					}}
				/>
			...
			>
				...
				<label style={fieldStyle}>
					...
					<select
						...
						value={loading}
						onChange={(event) =>
							setLoading(event.target.value as "lazy" | "eager")
						}
					>
				...
				<label style={fieldStyle}>
					...
					<select
						...
						onChange={(event) =>
							setPlaceholderType(event.target.value as "blurhash" | "url")
						}
					>
				...
				<label style={fieldStyle}>
					...
					<select
						style={inputStyle}
						value={objectFit}
						onChange={(event) => setObjectFit(event.target.value)}
					>
						<option>cover</option>
				...
				<label>
					<input
						type="checkbox"
						checked={placeholderFill}
						onChange={(event) => setPlaceholderFill(event.target.checked)}
					/>{" "}
					placeholder-fill
				</label>
				<label>
